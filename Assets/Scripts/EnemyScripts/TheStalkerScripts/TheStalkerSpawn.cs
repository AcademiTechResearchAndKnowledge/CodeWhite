...
public class IdleOrClosetEnemySpawner : MonoBehaviour
{
    ...
    private AggroEntityDetector[] standardEnemies;
    private DespawningEntityDetector[] despawningEnemies;

    private void Start()
    {
        FindPlayerReference();

        standardEnemies = FindObjectsByType<AggroEntityDetector>(FindObjectsInactive.Include, FindObjectsSortMode.None);
        despawningEnemies = FindObjectsByType<DespawningEntityDetector>(FindObjectsInactive.Include, FindObjectsSortMode.None);
    }
    ...
    private bool IsPlayerBeingHunted()
    {
        if (standardEnemies != null)
        {
            foreach (var enemy in standardEnemies)
            {
                if (enemy != null && enemy.IsActivelyHunting)
                {
                    return true;
                }
            }
        ...
        if (despawningEnemies != null)
        {
            foreach (var enemy in despawningEnemies)
            {
                if (enemy != null && enemy.IsActivelyHunting)
                {
                    return true;
